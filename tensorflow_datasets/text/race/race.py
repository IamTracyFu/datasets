...
class Race(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager: tfds.download.DownloadManager):
    ...
    path = os.path.join(path, "RACE")

    return {
        "train":
            self._generate_examples(
                os.path.join(path, "train"), task=self.builder_config.name),
        "dev":
            self._generate_examples(
                os.path.join(path, "dev"), task=self.builder_config.name),
        "test":
            self._generate_examples(
                os.path.join(path, "test"), task=self.builder_config.name),
    }

  def _generate_examples(self, path, task):
    """Yields examples."""
    if task == _HIGH:
      path = os.path.join(path, "high/*")
    else:
